...
*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;font-family:Arial,sans-serif}body{background:#f5f5f7;color:#111;height:100vh;height:100dvh;overflow:hidden}.hidden{display:none!important}button,input{font:inherit}button{cursor:pointer}
...
#app{height:100vh;height:100dvh;display:flex;flex-direction:column}header{height:58px;flex:0 0 58px;background:#fff;border-bottom:1px solid #ddd;display:flex;align-items:center;padding:0 14px;gap:12px}.hamb{border:0;background:transparent;font-size:25px;color:#111}.brand{font-weight:bold;font-size:21px;flex:1}.headuser{font-size:13px;color:#777}.logout{border:1px solid #ddd;background:#eee;border-radius:9px;padding:7px 10px}
#workspace{display:flex;min-height:0;flex:1;position:relative}#side{width:285px;background:#fff;border-right:1px solid #ddd;display:flex;flex-direction:column;transition:transform .2s;z-index:20}.sideTop{padding:14px;border-bottom:1px solid #ddd}.sideTitle{font-weight:bold;font-size:18px;margin-bottom:10px}.sideBtn{width:100%;text-align:left;border:1px solid #ddd;background:#f3f3f5;color:#111;border-radius:11px;padding:11px;margin:4px 0}.sideSection{padding:12px 14px 6px;color:#555;font-size:12px;text-transform:uppercase}.history{overflow:auto;flex:1;padding:0 10px}.chatItem{width:100%;border:1px solid transparent;background:#fff;color:#111;text-align:left;padding:11px;border-radius:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.chatItem:hover{background:#f2f2f4;border-color:#ddd}.newChat{margin:12px;border:0;background:#1677ff;color:#fff;border-radius:12px;padding:13px}
#main{min-width:0;flex:1;display:flex;flex-direction:column;min-height:0}.message-row{width:min(820px,100%);margin:12px auto;display:flex;align-items:flex-start;gap:6px}.message-row .message{max-width:none;margin:0;flex:1}.message{padding:14px 17px;border-radius:18px;line-height:1.5;white-space:pre-wrap;border:1px solid #ddd}.user{background:#1677ff;color:#fff;border-color:#1677ff}.stella{background:#fff;color:#111;box-shadow:0 2px 10px #0001}.copy-btn{flex:0 0 auto;width:34px;height:34px;margin-top:2px;border:1px solid #333;background:#fff;color:#111;border-radius:8px;font-size:16px;line-height:1}.copy-btn.copied{font-size:14px}.image-message{max-width:820px;margin:12px auto;background:#fff;color:#111;padding:10px;border-radius:18px;border:1px solid #ddd;box-shadow:0 2px 10px #0001}.generated-image{width:100%;max-height:700px;object-fit:contain;border-radius:12px}.image-actions{display:flex;gap:8px;margin-top:8px}.image-actions a,.image-actions button{border:1px solid #ddd;background:#eee;color:#111;border-radius:9px;padding:9px 12px;text-decoration:none}
...
@media(max-width:720px){#side{position:absolute;height:100%;transform:translateX(-100%);box-shadow:8px 0 30px #0002}#side.open{transform:translateX(0)}.headuser{display:none}.message-row{width:100%}.copy-btn{width:32px;height:32px}.chatItem{padding:12px 11px}}
</style></head><body>
...
function toggleSide(){$('side').classList.toggle('open')}function toggleTools(){$('tools').classList.toggle('hidden')}
function closePanels(){['settings','gallery','plugins','updates'].forEach(id=>$(id).classList.add('hidden'));$('pluginDetail').classList.add('hidden');$('pluginList').classList.remove('hidden')}
function openSettings(){closePanels();$('settings').classList.remove('hidden')}
...
function renderWelcome(){ $('chat').innerHTML=''; add("Hey! I'm STELLA.\nWhat are we doing today?",'stella',false)}
...
function add(t,type,doScroll=true){let row=document.createElement('div');row.className='message-row '+type+'-row';let m=document.createElement('div');m.className='message '+type;m.textContent=t;row.appendChild(m);if(type==='stella'){let copy=document.createElement('button');copy.className='copy-btn';copy.type='button';copy.title='Copy message';copy.setAttribute('aria-label','Copy STELLA message');copy.textContent='⧉';copy.onclick=async()=>{try{await navigator.clipboard.writeText(t);copy.textContent='✓';copy.classList.add('copied');setTimeout(()=>{copy.textContent='⧉';copy.classList.remove('copied')},1200)}catch(e){copy.textContent='!';setTimeout(()=>copy.textContent='⧉',1200)}};row.appendChild(copy)}$('chat').appendChild(row);if(doScroll)scroll();return m}
...
function renderGallery(items){let g=$('galleryGrid');g.innerHTML='';items.forEach(x=>{let t=document.createElement('div');t.className='tile';t.dataset.search=(x.prompt||'STELLA media').toLowerCase();t.innerHTML=x.type==='video'?'<video controls></video>':'<img>';let media=t.querySelector('img,video');media.src=x.url;t.innerHTML+='<small>'+esc(x.prompt||'STELLA media')+'</small>';g.appendChild(t)})}
