...
*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#f5f5f7;color:#111;height:100vh;overflow:hidden}.hidden{display:none!important}button,input{font:inherit}button{cursor:pointer}
...
#app{height:100vh;display:flex;flex-direction:column}header{height:58px;background:#fff;border-bottom:1px solid #ddd;display:flex;align-items:center;padding:0 14px;gap:12px}.hamb{border:0;background:transparent;font-size:25px;color:#111}.brand{font-weight:bold;font-size:21px;flex:1}.headuser{font-size:13px;color:#777}.logout{border:1px solid #ddd;background:#eee;border-radius:9px;padding:7px 10px}
...
function openPlugin(type){$('pluginList').classList.add('hidden');$('pluginDetail').classList.remove('hidden');let title=type==='image'?'Image generation':type==='voice'?'Voice call':'Video call';let action=type==='image'?"imagePicker()":type==='voice'?"startVoiceCall()":"alert('Video call is coming soon.')";$('pluginDetail').innerHTML='<button class="back" onclick="openPlugins()">Back to plugins</button><div class="pluginOption"><span>'+title+'</span><button onclick="'+action+'">Use</button></div>'}
