...
*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;background:#f5f5f7;color:#111;height:100vh;overflow:hidden}.hidden{display:none!important}button,input{font:inherit}
#auth{height:100vh;display:grid;place-items:center;padding:20px;background:#f5f5f7}.card{width:min(400px,100%);background:#fff;padding:30px;border-radius:24px;box-shadow:0 12px 40px #0002;text-align:center}.card input{width:100%;padding:14px;margin:6px 0;border:1px solid #ccc;border-radius:12px}.card button{width:100%;padding:14px;margin-top:10px;border:0;border-radius:12px;background:#1677ff;color:#fff}.switch{margin-top:16px;color:#1677ff;cursor:pointer}.error{color:#c00;min-height:20px;margin-top:8px}
#app{height:100vh;display:flex;flex-direction:column}header{height:58px;background:#fff;border-bottom:1px solid #ddd;display:flex;align-items:center;padding:0 14px;gap:12px}.hamb{border:0;background:transparent;font-size:25px;cursor:pointer}.brand{font-weight:bold;font-size:21px;flex:1}.headuser{font-size:13px;color:#777}.logout{border:0;background:#eee;border-radius:9px;padding:7px 10px}
#workspace{display:flex;min-height:0;flex:1;position:relative}#side{width:285px;background:#fff;border-right:1px solid #ddd;display:flex;flex-direction:column;transition:transform .2s;z-index:20}.sideTop{padding:14px;border-bottom:1px solid #eee}.sideTitle{font-weight:bold;font-size:18px;margin-bottom:10px}.sideBtn{width:100%;text-align:left;border:0;background:#f3f3f5;color:#111;border-radius:11px;padding:11px;margin:4px 0;cursor:pointer}.sideSection{padding:12px 14px 6px;color:#555;font-size:12px;text-transform:uppercase}.history{overflow:auto;flex:1;padding:0 10px}.chatItem{width:100%;border:0;background:#fff;color:#111;text-align:left;padding:11px;border-radius:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.chatItem:hover{background:#f2f2f4}.newChat{margin:12px;border:0;background:#1677ff;color:#fff;border-radius:12px;padding:13px;cursor:pointer}
#main{min-width:0;flex:1;display:flex;flex-direction:column}#chat{flex:1;overflow:auto;padding:20px}.message{max-width:820px;margin:12px auto;padding:14px 17px;border-radius:18px;line-height:1.5;white-space:pre-wrap}.user{background:#1677ff;color:#fff}.stella{background:#fff;color:#111;box-shadow:0 2px 10px #0001}.image-message{max-width:820px;margin:12px auto;background:#fff;color:#111;padding:10px;border-radius:18px;box-shadow:0 2px 10px #0001}.generated-image{width:100%;max-height:700px;object-fit:contain;border-radius:12px}.image-actions{display:flex;gap:8px;margin-top:8px}.image-actions a,.image-actions button{border:0;background:#eee;color:#111;border-radius:9px;padding:9px 12px;text-decoration:none}
#inputArea{display:flex;gap:8px;padding:14px;background:#fff;border-top:1px solid #ddd;align-items:center}.plus{height:46px;width:46px;border:0;border-radius:14px;background:#eee;color:#111;font-size:25px}.input{flex:1;padding:14px;border:1px solid #ccc;border-radius:15px;outline:0;background:#fff;color:#111}.send{height:46px;border:0;border-radius:14px;background:#1677ff;color:#fff;padding:0 18px}.tools{position:absolute;bottom:68px;left:12px;background:#fff;color:#111;border:1px solid #ddd;border-radius:15px;padding:8px;box-shadow:0 8px 30px #0003;z-index:30}.tools button{display:block;width:220px;border:0;background:#fff;color:#111;text-align:left;padding:12px;border-radius:10px}
#settings,#gallery,#plugins,#updates{position:absolute;inset:0;background:#f5f5f7;color:#111;z-index:25;padding:18px;overflow:auto}.panel{max-width:900px;margin:auto}.panelHead{display:flex;align-items:center;gap:12px;margin-bottom:15px}.back{border:0;background:#fff;color:#111;border-radius:10px;padding:9px 12px}.setting,.pluginCard,.updateCard{background:#fff;color:#111;border-radius:15px;padding:15px;margin:10px 0;display:flex;justify-content:space-between;align-items:center}.setting select,.setting input{padding:9px;border:1px solid #ccc;border-radius:9px}.galleryGrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(170px,1fr));gap:12px}.tile{background:#fff;color:#111;border-radius:14px;padding:8px}.tile img,.tile video{width:100%;aspect-ratio:1;object-fit:cover;border-radius:10px}.tile small{display:block;padding:6px}.pluginCard{display:block}.pluginCard strong{display:block;font-size:17px;margin-bottom:5px}.pluginCard span{color:#555;font-size:14px}.updateCard{display:block}.updateDate{font-size:12px;color:#666;margin-bottom:5px}.updateCard strong{display:block;margin-bottom:6px}.updateCard p{margin:0;line-height:1.5;color:#333}
#call{position:absolute;inset:0;background:#111;color:#fff;z-index:40;display:flex;flex-direction:column;align-items:center;justify-content:center}.orb{width:130px;height:130px;border-radius:50%;background:#fff1;display:grid;place-items:center;font-size:55px;margin:20px}.callStatus{color:#bbb}.endCall{margin-top:35px;width:68px;height:68px;border:0;border-radius:50%;background:#e53935;color:#fff;font-size:25px}.callTimer{font-variant-numeric:tabular-nums;margin-top:8px}
/* STELLA dark mode: the surface becomes black, while the Gemini logo/brand, blue messages and readable text stay bright. */
body.dark,#auth.dark{background:#000;color:#fff}body.dark header,body.dark #side,body.dark #inputArea,body.dark .card{background:#000;color:#fff;border-color:#222}body.dark .sideBtn,body.dark .plus,body.dark .logout{background:#111;color:#fff;border-color:#222}body.dark .chatItem{background:#000;color:#fff}body.dark .chatItem:hover{background:#111}body.dark .sideSection{color:#aaa}body.dark .stella,body.dark .image-message,body.dark .tools,body.dark .tools button,body.dark #settings,body.dark #gallery,body.dark #plugins,body.dark #updates,body.dark .setting,body.dark .pluginCard,body.dark .updateCard,body.dark .back{background:#000;color:#fff;border-color:#222}body.dark .stella,body.dark .image-message{box-shadow:none}body.dark .image-actions a,body.dark .image-actions button{background:#111;color:#fff}body.dark .input{background:#000;color:#fff;border-color:#444}body.dark .input::placeholder{color:#aaa}body.dark .card input{background:#000;color:#fff;border-color:#444}body.dark .card input::placeholder{color:#aaa}body.dark .updateDate,body.dark .pluginCard span,body.dark .updateCard p{color:#ccc}body.dark .brand{color:#fff}body.dark .headuser{color:#fff}
...
<div id="app" class="hidden"><header><button class="hamb" onclick="toggleSide()">☰</button><div class="brand">✦ STELLA</div><span class="headuser" id="welcome"></span><button class="logout" onclick="logout()">Log out</button></header><div id="workspace"><aside id="side"><div class="sideTop"><div class="sideTitle">STELLA</div><button class="sideBtn" onclick="openGallery()">▦ Memory</button><button class="sideBtn" onclick="openSettings()">⚙ Settings</button><button class="sideBtn" onclick="openPlugins()">＋ Plugins</button><button class="sideBtn" onclick="openUpdates()">▣ Update Log</button></div><div class="sideSection">Previous chats</div><div id="history" class="history"></div><button class="newChat" onclick="newChat()">＋ New chat</button></aside><main id="main"><div id="chat"></div><div id="inputArea"><button class="plus" onclick="toggleTools()">＋</button><input id="message" class="input" placeholder="Message STELLA..." autocomplete="off"><button class="send" onclick="sendMessage()">Send</button></div><div id="tools" class="tools hidden"><button onclick="imagePicker()">Generate an image</button><button onclick="startVoiceCall()">Voice call with STELLA</button></div></main><section id="settings" class="hidden"><div class="panel"><div class="panelHead"><button class="back" onclick="closePanels()">Back</button><h2>Settings</h2></div><div class="setting"><span>Theme</span><select id="theme" onchange="saveSettings()"><option value="system">According to device</option><option value="light">Light</option><option value="dark">Dark</option></select></div><div class="setting"><span>Memory across new chats</span><input id="memory" type="checkbox" onchange="saveSettings()"></div></div></section><section id="gallery" class="hidden"><div class="panel"><div class="panelHead"><button class="back" onclick="closePanels()">Back</button><h2>Memory gallery</h2></div><div id="galleryGrid" class="galleryGrid"></div></div></section><section id="plugins" class="hidden"><div class="panel"><div class="panelHead"><button class="back" onclick="closePanels()">Back</button><h2>Plugins</h2></div><div class="pluginCard"><strong>Image generation</strong><span>Generate images with STELLA using FLUX.</span></div><div class="pluginCard"><strong>Voice call</strong><span>Talk with STELLA using the configured Gemini voice service.</span></div><div class="pluginCard"><strong>More plugins coming</strong><span>This page is the home for future STELLA capabilities.</span></div></div></section><section id="updates" class="hidden"><div class="panel"><div class="panelHead"><button class="back" onclick="closePanels()">Back</button><h2>Update Log</h2></div><div class="updateCard"><div class="updateDate">Latest patch</div><strong>STELLA v3 interface update</strong><p>Dark mode is now a true dark interface with readable white text. The Plugins button now opens a dedicated Plugins page, and this Update Log makes new changes easier to understand.</p></div></div></section><section id="call" class="hidden"><div class="orb">✦</div><h2>STELLA</h2><div class="callStatus" id="callStatus">Connecting...</div><div class="callTimer" id="callTimer">00:00</div><button class="endCall" onclick="endCall()">×</button></section></div></div>
<script>
...
function closePanels(){['settings','gallery','plugins','updates'].forEach(id=>$(id).classList.add('hidden'))}
function openSettings(){closePanels();$('settings').classList.remove('hidden');loadSettings()}
function openGallery(){closePanels();$('gallery').classList.remove('hidden');loadGallery()}
function openPlugins(){closePanels();$('plugins').classList.remove('hidden');if(innerWidth<721)$('side').classList.remove('open')}
function openUpdates(){closePanels();$('updates').classList.remove('hidden');if(innerWidth<721)$('side').classList.remove('open')}
