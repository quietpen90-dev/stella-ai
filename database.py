...
def verify_password(password, stored_password):
    ...
    password_hash = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), bytes.fromhex(salt_hex), 200_000)
    return secrets.compare_digest(password_hash, bytes.fromhex(stored_hash_hex))


def verify_user(username, password):
    connection = get_connection(); cursor = connection.cursor()
    cursor.execute("SELECT id, password FROM users WHERE username = ?", (username,)); user = cursor.fetchone(); connection.close()
    if user is None or not verify_password(password, user[1]): return None
    return user[0]


def create_session(user_id):
    token = secrets.token_urlsafe(32); connection = get_connection()
    connection.execute("INSERT INTO sessions (token, user_id) VALUES (?, ?)", (token, user_id)); connection.commit(); connection.close(); return token


def get_user_from_session(token):
    if not token: return None
    connection = get_connection(); row = connection.execute("SELECT user_id FROM sessions WHERE token = ?", (token,)).fetchone(); connection.close(); return row[0] if row else None


def delete_session(token):
    if not token: return
    connection = get_connection(); connection.execute("DELETE FROM sessions WHERE token = ?", (token,)); connection.commit(); connection.close()


def get_username(user_id):
    connection = get_connection(); row = connection.execute("SELECT username FROM users WHERE id = ?", (user_id,)).fetchone(); connection.close(); return row[0] if row else None


def create_conversation(user_id, title="New chat"):
    connection = get_connection(); cursor = connection.cursor(); cursor.execute("INSERT INTO conversations (user_id, title) VALUES (?, ?)", (user_id, title[:100])); connection.commit(); cid = cursor.lastrowid; connection.close(); return cid


def get_conversations(user_id):
    connection = get_connection(); rows = connection.execute("SELECT id, title, created_at, updated_at FROM conversations WHERE user_id = ? ORDER BY updated_at DESC, id DESC", (user_id,)).fetchall(); connection.close()
...
def get_user_settings(user_id):
    connection=get_connection(); row=connection.execute("SELECT theme,memory_enabled FROM user_settings WHERE user_id=?",(user_id,)).fetchone()
    if row is None: connection.execute("INSERT OR IGNORE INTO user_settings (user_id) VALUES (?)",(user_id,)); connection.commit(); result={"theme":"system","memory_enabled":True}
    else: result={"theme":row[0],"memory_enabled":bool(row[1])}
    connection.close(); return result


def update_user_settings(user_id, theme=None, memory_enabled=None):
    current=get_user_settings(user_id); theme=theme if theme in ("system","light","dark") else current["theme"]; memory_enabled=current["memory_enabled"] if memory_enabled is None else bool(memory_enabled); connection=get_connection(); connection.execute("INSERT INTO user_settings (user_id,theme,memory_enabled) VALUES (?,?,?) ON CONFLICT(user_id) DO UPDATE SET theme=excluded.theme,memory_enabled=excluded.memory_enabled",(user_id,theme,1 if memory_enabled else 0)); connection.commit(); connection.close(); return {"theme":theme,"memory_enabled":memory_enabled}
...
def delete_conversation(user_id,conversation_id):
    connection=get_connection(); connection.execute("DELETE FROM conversations WHERE id=? AND user_id=?",(conversation_id,user_id)); connection.commit(); connection.close()


def create_voice_call(user_id,conversation_id,provider=None,provider_session_id=None):
    connection=get_connection(); cursor=connection.cursor(); cursor.execute("INSERT INTO voice_calls (user_id,conversation_id,provider,provider_session_id,status) VALUES (?,?,?,?, 'active')",(user_id,conversation_id,provider,provider_session_id)); connection.commit(); cid=cursor.lastrowid; connection.close(); return cid


def get_voice_call(call_id,user_id):
    connection=get_connection(); row=connection.execute("SELECT id,conversation_id,provider,provider_session_id,status,started_at,ended_at FROM voice_calls WHERE id=? AND user_id=?",(call_id,user_id)).fetchone(); connection.close()
    if not row:return None
    return {"id":row[0],"conversation_id":row[1],"provider":row[2],"provider_session_id":row[3],"status":row[4],"started_at":row[5],"ended_at":row[6]}


def end_voice_call(call_id,user_id):
    connection=get_connection(); connection.execute("UPDATE voice_calls SET status='ended',ended_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=?",(call_id,user_id)); connection.commit(); connection.close()
